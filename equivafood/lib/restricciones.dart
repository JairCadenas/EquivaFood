...
const List<RestriccionItem> catalogoRestricciones = [
  ...
  RestriccionItem(
    tag: 'pescado',
    label: 'Mariscos/Pescado',
    icono: Icons.set_meal_outlined,
    categoria: 'Alergias',
  ),
  RestriccionItem(
    tag: 'carne',
    label: 'Sin carne roja',
...
class RestriccionesWidget extends StatefulWidget {
  ...
  final bool
  preferencia; // CORREGIDO: cambiado de preferenciaCarne a preferencia
...
class _RestriccionesWidgetState extends State<RestriccionesWidget> {
  ...
  void _toggleTag(String tag) {
    setState(() {
      _seleccionadas.contains(tag)
          ? _seleccionadas.remove(tag)
          : _seleccionadas.add(tag);
    });
  ...
  Widget build(BuildContext context) {
    ...
    final categorias = catalogoRestricciones
        .map((e) => e.categoria)
        .toSet()
        .toList();
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: _dietaCard(
                label: 'Omnívora',
                sublabel: 'Incluye carne y pescado',
                icono: Icons.restaurant,
                seleccionado: _preferencia, // CORREGIDO
                onTap: () => _setDieta(true),
                color: primaryColor,
              ),
            ),
            ...
            Expanded(
              child: _dietaCard(
                label: 'Vegetal',
                sublabel: 'Sin carne ni pescado',
                icono: Icons.eco,
                seleccionado: !_preferencia, // CORREGIDO
                onTap: () => _setDieta(false),
                color: Colors.green,
              ),
            ),
          ],
        ...
        ...categorias.map((categoria) {
          final items = catalogoRestricciones
                      ...
                      child: Row(
                        ...
                        children: [
                          Icon(
                            item.icono,
                            size: 16,
                            color: activo ? Colors.redAccent : Colors.grey,
                          ),
...
class _RestriccionesScreenState extends State<RestriccionesScreen> {
  ...
  Future<void> _cargarPreferencias() async {
    ...
    setState(() {
      ...
      _preferencia =
          prefs.getBool('preferencia') ?? true; // CORREGIDO: clave local
      _isLoading = false;
    });
  }
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Tus alternativas del menú se ajustarán automáticamente según estas preferencias.',
